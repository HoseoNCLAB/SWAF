...
void process_rule(char *buf, json_t *rules_obj) {
    ...
    char *raw = op_regex; if (*raw == '!') raw++;
    char *operator_name, *regex_str;
    if (*raw == '@') {
        char *space = strchr(raw, ' ');
        if (space) {
            operator_name = strndup(raw + 1, space - (raw + 1));
            regex_str = strdup(space + 1);
        } else {
            operator_name = strdup(raw + 1);
            regex_str = strdup("");
        }
    } else {
        operator_name = strdup("");
        regex_str = strdup(raw);
    }
    free(op_regex);
...
int main(int argc, char *argv[]) {
    ...
    if (json_dump_file(rules, "parsed_rules12.json", JSON_INDENT(2)) < 0) {
