                                              ...
                                              uint32_t pid)
{
    ...
    for (; t != NULL; t = t->next) {
        ...
        if (t->id == pid && t->offset == offset && t->depth == depth) {
            SWAF_BUG_ON(t->len != patlen);
            SWAF_BUG_ON(SCMemcmp(t->original_pat, pat, patlen) != 0);
            return t;
        }
    }
                          ...
                          SigIntId sid, uint32_t flags)
{
    ...
    if (p == NULL) {
        ...
        if (!(mpm_ctx->flags & MPMCTX_FLAGS_NODEPTH)) {
            if (depth) {
                mpm_ctx->maxdepth = SWAF_MAX(mpm_ctx->maxdepth, depth);
...
{
    SWAF_BUG_ON(p->original_pat == NULL);
    SWAF_BUG_ON(p->sids == NULL);
...
static int PatternDatabaseCompile(PatternDatabase *pd, SCHSCompileData *cd)
{
    for (uint32_t i = 0; i < pd->pattern_cnt; i++) {
        ...
        // 정규식을 그대로 복사 (리터럴이 아닌 진짜 정규식으로)
        cd->expressions[i] = SCCalloc(1, p->len + 1, char);
        if (cd->expressions[i] == NULL) {
            return -1;
        }
        memcpy(cd->expressions[i], p->original_pat, p->len);
    ...
    hs_error_t err = hs_compile_ext_multi(
        (const char *const *)cd->expressions,
        cd->flags,
        cd->ids,
        (const hs_expr_ext_t *const *)cd->ext,
        cd->pattern_cnt,
        HS_MODE_BLOCK,
        NULL,
        &pd->hs_db,
...
int SCHSPreparePatterns(MpmConfig *mpm_conf, MpmCtx *mpm_ctx)
{
    ...
    /* Serialize whole database compilation to ensure dedupe is safe */
    ...
    SWAF_BUG_ON(ctx->pattern_db != NULL); /* already built? */
    SWAF_BUG_ON(mpm_ctx->pattern_cnt == 0);
    ...
    printf("[INFO] Hyperscan 컴파일 완료: 총 등록 패턴 수 = %u\n\n", pd->pattern_cnt);
                            ...
                            void *ctx)
{
    ...
    // 방어: PatternDatabase 및 parray 유효성 확인
    if (pd == NULL || pd->parray == NULL || id >= pd->pattern_cnt) {
        fprintf(stderr, "[FATAL] Invalid PatternDatabase or index (id=%u)\n", id);
        return -1;
    }

    const SCHSPattern *pat = pd->parray[id];
    if (pat == NULL) {
        fprintf(stderr, "[FATAL] pat is NULL at id=%u\n", id);
        return -1;
    }

    // 디버그 출력
    printf("[MATCH CALLBACK] Pattern ID=%u, pat->id=%u, sids_size=%u\n",
    id, pat->id, pat->sids_size);

    // 방어: sids가 NULL인데 size가 있는 경우 segfault 가능
    if (pat->sids == NULL && pat->sids_size > 0) {
        fprintf(stderr, "[FATAL] pat->sids is NULL but sids_size=%u\n", pat->sids_size);
        return -1;
    }

    // Prefilter에 룰 ID 등록
    if (pat->sids && pat->sids_size > 0) {
        PrefilterAddSids(pmq, pat->sids, pat->sids_size);
    }

    cctx->match_count++;

    printf("[MATCH] Hyperscan matched! Pattern ID = %u\n", id);
    return 0;
}
                    ...
                    PrefilterRuleStore *pmq, const uint8_t *buf, const uint32_t buflen)
{
    ...
    SWAF_BUG_ON(pd->hs_db == NULL);
    SWAF_BUG_ON(scratch == NULL);

    printf("[DEBUG] hs_scan 시작 - buflen: %u, payload: \"%.*s\"\n", buflen, buflen, buf);
